...
const inputShape = {
  ...
  includeHistorical: z
    .boolean()
    .default(false)
    .describe("If true, include members already removed (datumVymazu set). Default: only currently active."),
};
...
export const getStatutoryBodiesTool = defineTool({
  ...
  inputShape,
  handler: async ({ ico, includeHistorical }, { client }) => {
    try {
